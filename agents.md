...
| `freminal-damage-model`            | Deriving damage: `*_changed` signals feeding `PaneFrameDamage` (per-pane) -> `FrameDamage` (terminal-wide) -> `ChromeDamage` composition -> windowing's `PresentRegion` / `DamageHistory`; or clear/clip/present-region/full-repaint logic. Classify before using `Full`.                                                                                             |
