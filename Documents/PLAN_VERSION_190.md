# PLAN_VERSION_190.md — v0.19.0 "AI Assist — Generative"
...
shared AI foundation (endpoint client, consent, key storage) established in v0.18.0
(Task 87a).

Depends on Task 87a (v0.18.0 Advisory — the endpoint/consent/key foundation must be solid
...
- **Pipeline explanation (reverse)** — explain a gnarly pasted one-liner before running it
  (sits between advisory and generative; include here or in v0.18.0 at activation).
...
- Whether streaming responses (typewriter effect) are added here vs kept blocking. (Lean:
  ...
  stream complexity.)
- Whether `keychain` key storage (if deferred from v0.18.0) is completed here.
...
**All invariants from `PLAN_VERSION_180.md` apply unchanged.** The ones that bite hardest
...
See `PLAN_VERSION_180.md` for the full, authoritative invariant list and the shared
...
- **Generation rides on the proven Advisory foundation.** The endpoint client, consent
  ...
  v0.18.0. This version adds the generation features and (optionally) streaming.
- **The reviewable-surface rule is the safety model.** Everything generative produces
  ...
  of scope.
- **Out of scope (carried from v0.18.0):** ghost-text autocomplete, suggest-next-command,
  agentic execution.
