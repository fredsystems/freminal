# PLAN_VERSION_150.md — v0.15.0 "Remote"
...
(v0.20.0), so SSH v1 has no scripting hooks; hook integration is retrofitted when Task 84
lands.
...
- Authentication (key delegation, agent forwarding, OS keychain integration — note the
  `keyring` 4.x decisions captured for AI key storage in `PLAN_VERSION_180.md` may apply
  here too).
...
- **SSH ships non-scriptable.** The earlier plan coupled SSH's security boundary to the
  ...
  (Task 84, v0.20.0) retrofits remote-connection hooks later.
