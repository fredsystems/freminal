...
Last updated: 2026-07-05 — Task 114 (v0.11.0): the previously egui-blocked
keyboard keys are now delivered. Keypad operators/directional, media keys,
and lock/print/pause/menu-as-keys are delivered via a raw-winit intercept
(`App::on_raw_key_event` in `freminal-windowing`) and encoded through the
existing KKP `CSI u` path; true `caps_lock`/`num_lock` modifier bits (64/128)
are sourced from an OS lock-state query — `evdev`/EVIOCGLED on Linux (one
path for X11 and Wayland), `GetKeyState` on Windows,
`CGEventSourceFlagsState` on macOS (caps only) — read at cold-start and
focus-gain. Kitty keyboard is now near-complete; the only remaining gaps are
ISO_Level3/5_Shift (no winit `KeyCode` variant) and the hyper/meta modifier
bits (no platform source on any target). Same day — Task 101 (v0.11.0),
...
| CSI > u       | Kitty Keyboard: push flags          | ✅     | Kitty keyboard protocol (Task 35); functional keys emit `:event-type` on repeat/release under flag 2. Super modifier (bit 8), F13–F35 (`CSI 57376 u`…), modifier-keys-as-keys (Shift/Control/Alt/Super Left/Right under flag 8), F3 normalized to `CSI 13 ~` (Task 101). Task 114 delivered the previously egui-blocked keypad/media/lock/print/pause/menu keys (raw-winit intercept) and true caps_lock/num_lock bits (OS lock-state query); only ISO_Level3/5_Shift (no winit `KeyCode` variant) and hyper/meta bits (no platform source) remain unimplemented |
...
| Kitty Keyboard Protocol         | ✅              | 🚧                 | Task 35 + Task 101: super modifier, F13–F35, modifier-keys-as-keys (flag 8), F3 → `CSI 13 ~`. Task 114 delivered keypad/media/lock/print/pause/menu keys (raw-winit intercept) + true caps_lock/num_lock bits (OS lock-state query). Remaining: ISO_Level3/5_Shift (no winit `KeyCode`), hyper/meta bits (no platform source) |
