...
v0.11.0): super modifier bit (101.2); F13–F35, modifier-keys-as-keys under
flag 8, and F3 → `CSI 13 ~` normalization (101.3). Kitty keyboard is now
"substantially compliant"; the egui-blocked remainder (keypad/media/ISO/lock/
print/pause/menu keys + true caps_lock/num_lock state) is tracked in Task 114.
Earlier: 2026-07-02 — Kitty graphics render-path fixes (Tasks
...
| CSI > u       | Kitty Keyboard: push flags          | ✅     | Kitty keyboard protocol (Task 35); functional keys emit `:event-type` on repeat/release under flag 2. Substantially compliant after Task 101: super modifier (bit 8), F13–F35 (`CSI 57376 u`…), modifier-keys-as-keys (Shift/Control/Alt/Super Left/Right under flag 8), F3 normalized to `CSI 13 ~`. egui-blocked remainder (keypad/media/ISO/lock/print/pause/menu keys + caps_lock/num_lock state) tracked in Task 114 |
...
| Kitty Keyboard Protocol         | ✅              | 🚧                 | Task 35 + Task 101 (substantially compliant): super modifier, F13–F35, modifier-keys-as-keys (flag 8), F3 → `CSI 13 ~`. egui-blocked keys (keypad/media/ISO/lock/print/pause/menu + caps_lock/num_lock) tracked in Task 114 |
