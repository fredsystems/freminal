...
overhaul (ligature + powerline coverage out of the box) solve the same problem — the bare-bones
visual baseline — from different angles, and are highly intertwined, so they ship together. They
land **before** the AI-assist versions (v0.18.0–v0.19.0) deliberately: those versions introduce
more modals and overlays, and every new modal inherits whatever visual baseline exists. Paying
...
into v0.14.0–v0.16.0 and v0.20.0) and remaining Category C housekeeping (Tasks 18, 19). A.2
...
| 123 | GL Pipeline Measurement Harness           | `PLAN_123_GL_MEASUREMENT_HARNESS.md`          | In progress | Task 122             |
...
**Tasks 99–101, 114 (v0.11.0, kitty notifications & graphics finish):** Task 99 (OSC 99) builds
on the OSC 9/777 notification routing from Task 76 and extends it with a stateful identity map
and a reverse-PTY-write activation path (which already exists via `write_to_pty` /
`Pane::pty_write_tx`). Task 100 (graphics completion) extends the Task 13 kitty graphics
handler; the 2026-07-01 100.1 audit found the parser types most keys but **not** the
relative-placement keys `P/Q/H/V` (added in foundation subtask 110.0), and confirmed animation,
`t=s`, `o=z`, delete-target correctness, and z-index ordering as the remaining work. Keyboard
...
is dropped. The event hook API (Task 84) instead lands last (v0.20.0) and may _add_ an AI hook
among its event integrations.

**Status bar (Task 85) no longer depends on the event hook API.** It ships self-contained
(built-in segments + a shell-out segment type) in v0.17.0 and gains hook-driven segments later
when Task 84 lands.
...
### v0.6.0 — Foundation (eframe Replacement) — Complete

Replaced eframe with direct winit + glutin + egui integration in the new
`freminal-windowing` crate. Tasks 62–67. See `DESIGN_DECISIONS.md`
("`freminal-windowing` Crate") for the durable crate charter and design
decisions.

### v0.7.0 — Recording & Layouts — Complete

FREC v2 recording format (removing v1 and playback), saved layouts
(TOML-based session templates), platform performance triage, and UI polish.
Tasks 59, 61, 68, 69. See `FREC_FORMAT.md`, `LAYOUT_FORMAT.md`, and
`DESIGN_DECISIONS.md` for durable format specs and rationale.
...
| 123  | 2026-08-21 |            | Phase 1 (123.1-123.10, 123.14) done; Phase 2 blocked on `nix develop` for 123.10 |
...
- `Documents/PLAN_VERSION_170.md` — v0.17.0 "Status Bar" (stub, Task 85)
