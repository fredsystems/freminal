...
///
/// Move the cursor to the first column of the Ps-th line up (default = 1).
pub fn ansi_parser_inner_csi_finished_cpl(
    params: &[u8],
    output: &mut Vec<TerminalOutput>,
) -> ParserOutcome {
    let Ok(param) = parse_param_as::<i32>(params) else {
        return ParserOutcome::InvalidParserFailure(ParserFailures::UnhandledCPLCommand(
