...
fn scroll_region_moves_cursor_to_screen_origin() {
    ...
    assert_eq!(buf.cursor_screen_pos().y, 9);
...
fn lf_outside_region_above_moves_cursor() {
    ...
    let cursor_after = buf.cursor().pos.y;
...
fn resize_clamps_scroll_region_and_cursor() {
    ...
    assert_eq!(buf.cursor_screen_pos().y, 0); // DECSTBM homes to screen origin
    ...
    assert!(buf.cursor_screen_pos().y < 5);
}
...
fn scroll_region_persists_through_width_resize() {
    ...
    let region_cursor_y = buf.cursor().pos.y;
    ...
    assert_eq!(buf.cursor().pos.y, region_cursor_y);
}
...
fn multiple_scroll_up_operations() {
    ...
    assert_eq!(buf.cursor_screen_pos().y, 5);
...
fn multiple_scroll_down_operations() {
    ...
    assert_eq!(buf.cursor_screen_pos().y, 2);
...
fn reset_scroll_region_to_full_screen() {
    ...
    assert!(buf.rows().len() > initial_rows);
}
...
fn wide_character_at_scroll_boundary() {
    ...
    assert_eq!(buf.cursor().pos.x, 2); // Wide char takes 2 columns
}
