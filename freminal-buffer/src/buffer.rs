...
impl Buffer {
    ...
    /// a linear `(Vec<TChar>, Vec<FormatTag>, Vec<usize>)` triple using the
    /// same algorithm as [`Self::visible_as_tchars_and_tags`].
    ///
    /// Returns `(vec![], vec![], vec![])` for the alternate screen buffer,
    /// which never accumulates scrollback.
    ///
    /// Flatten all scrollback rows (everything before the visible window) into
    ...
    ///
    /// The returned `Vec<usize>` contains row offsets: `row_offsets[r]` is the
    /// flat index in `chars` where row `r` begins.  This enables O(1) row
    /// lookup in `flat_index_for_cell` instead of a linear scan for `NewLine`
    /// separators.
...
mod visible_as_tchars_and_tags_tests {
    ...
    fn empty_buffer_returns_single_default_tag() {
        ...
        let mut buf = Buffer::new(10, 5);
        let (chars, tags, _row_offsets) = buf.visible_as_tchars_and_tags(0);
    ...
    fn wide_char_no_continuation_in_output() {
        ...
        let (chars, _tags, _row_offsets) = buf.visible_as_tchars_and_tags(0);
