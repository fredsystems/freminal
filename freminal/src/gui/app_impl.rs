...
/// takes a `bool` (that file is out of scope here), so the one call site
/// inside `drain_command_finished_events` converts back via
/// [`Self::is_focused`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum WindowFocus {
...
impl WindowFocus {
    ...
    const fn from_bool(focused: bool) -> Self {
        if focused {
            Self::Focused
        } else {
            Self::Unfocused
...
fn drain_command_finished_events(
    tabs: &mut TabManager,
    tab_title_policy: freminal_common::config::TabTitlePolicy,
    tab_title_separator: &str,
    notifications_config: &freminal_common::config::NotificationsConfig,
    bell_config: &freminal_common::config::BellConfig,
...
mod tests {
    ...
    fn drain_command_finished_events_flags_only_the_non_active_tab() {
        ...
        let toasts = std::cell::RefCell::new(super::super::toast::ToastStack::default());
    ...
    fn drain_command_finished_events_routes_a_toast_when_focused_and_enabled() {
        ...
        assert_eq!(
            ...
            Some(super::super::toast::ToastKind::Info),
