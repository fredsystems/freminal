...
pub struct GlyphAtlas {
    ...
    /// Procedural box-drawing / block-element glyphs (Task #410), keyed by the
    /// source char alone. Cell dimensions are constant for the atlas's
    /// lifetime (a font-size or DPI change clears the whole atlas), so the
    /// char is a sufficient key. Stored separately from `entries` so it never
    /// collides with font `GlyphKey`s.
    procedural_entries: FxHashMap<char, AtlasEntry>,
...
impl GlyphAtlas {
    ...
    pub fn new(initial_size: u32, max_size: u32) -> Self {
        ...
        Self {
            ...
            procedural_entries: FxHashMap::default(),
            shelves: Vec::new(),
            generation: 0,
    ...
    /// Get or insert a procedural box-drawing / block-element glyph (Task
    /// #410).
    ///
    /// Unlike font glyphs, the bitmap is generated by
    /// [`crate::gui::box_drawing::generate_alpha`] at the exact cell pixel size
    /// (`w` × `h`) so it fills the cell edge-to-edge and tiles seamlessly.
    /// `bearing_y` positions the (full-cell) bitmap so the existing emit path
    /// places it at the cell top; the caller passes the baseline-to-cell-top
    /// distance.
    ///
    /// Returns `None` only if the glyph cannot be packed into the atlas.
    pub fn get_or_insert_procedural(
        ...
        let w_usize = usize_from_u32(w);
        let h_usize = usize_from_u32(h);
        let data = crate::gui::box_drawing::generate_alpha(ch, w_usize, h_usize);
        ...
        let rasterized = RasterizedGlyph {
            ...
            height: h,
            bearing_x: 0,
            bearing_y: i32::from(bearing_y),
            is_color: false,
        };

        // Reuse the shared shelf packer / UV computation / dirty-rect upload
        // machinery, then move the entry into the procedural map so it never
        // collides with a font `GlyphKey`.
        let entry = self.insert_rasterized_raw(&rasterized)?;
        self.procedural_entries.entry(ch).or_insert(entry);
        self.procedural_entries.get(&ch)
    }
    ...
    fn place_in_shelf(
        ...
        let entry = self.pack_glyph(shelf_idx, glyph);
        // Use entry API to insert and return a reference in one lookup.
        self.entries.entry(key).or_insert(entry)
    }

    /// Pack a rasterised glyph into `shelf_idx`: blit its pixels, record the
    /// dirty rect, compute the UV rect, and return the owned [`AtlasEntry`]
    /// (without inserting it into any lookup map — the caller decides which).
    fn pack_glyph(&mut self, shelf_idx: usize, glyph: &RasterizedGlyph) -> AtlasEntry {
        let gw = glyph.width;
        let gh = glyph.height;
    ...
    /// Pack a rasterised glyph into the atlas and return the owned entry,
    /// running the same shelf-find / evict / grow logic as
    /// [`Self::insert_rasterized`] but without keying it into `entries`. Used
    /// for procedural glyphs, which live in their own map.
    fn insert_rasterized_raw(&mut self, glyph: &RasterizedGlyph) -> Option<AtlasEntry> {
        let gw = glyph.width;
        let gh = glyph.height;
        ...
        Some(self.pack_glyph(shelf_idx, glyph))
    }
